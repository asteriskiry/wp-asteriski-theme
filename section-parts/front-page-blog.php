    <?php  
    $thebis_blog_title =  get_theme_mod('thebis_blog_title');  
    $thebis_blog_section = get_theme_mod('thebis_blog_section_hideshow','show');
    if ($thebis_blog_section =='show') { 
    ?>
    <section class="recent-posts gray-section large-section">
      <div class="container">
        <div class="container">
          <div class="row">
            <?php if($thebis_blog_title != "")
              {?>
            <div class="col d-flex justify-content-center">
              <h2 class="section-title text-center title-divider">
                <?php echo esc_html(get_theme_mod('thebis_blog_title')); ?>
              </h2>
            </div>
            <?php } ?>
          </div>
        ...
        <div class="row blog-wrapper">
          <?php 
            $thebis_latest_blog_posts = new WP_Query( array( 'posts_per_page' => 3 ) );
            if ( $thebis_latest_blog_posts->have_posts() ) : 
            while ( $thebis_latest_blog_posts->have_posts() ) : $thebis_latest_blog_posts->the_post(); 
           ?>
          <div class="col-lg-4">
